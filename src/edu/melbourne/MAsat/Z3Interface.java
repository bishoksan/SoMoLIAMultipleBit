...
//        System.out.println("received formula "+f);
        Tactic preamble = ctx.then(ctx.mkTactic("simplify"), ctx.mkTactic("propagate-values"),
                //ctx.mkTactic("solve-eqs"), //removes some variables unpredictably and so the result maynot be sound, be careful using it
                ctx.mkTactic("simplify"));
...
//        return (BoolExpr) iteFreeFormula.simplify();
    ...
    //makes a copy of a solver 
    public Solver copySolver(Solver s, Context ctx) {
...
//        }
        ...
        BoolExpr qfF = z3.eliminateQuantifiers(ctx, be);
