    ...
    public GRBModel addAConstrGRB(BoolExpr be, GRBModel model, String id, HashMap<IntExpr, GRBVar> z3GRBVarMap) {//be is an arith constraint
        if (!be.isTrue()) {
            ...
            try {
                ...
                GRBLinExpr grbExpr0 = z3Expr2GRBExpr((ArithExpr) exprs[0].simplify(), model, z3GRBVarMap);
                GRBLinExpr grbExpr1 = z3Expr2GRBExpr((ArithExpr) exprs[1].simplify(), model, z3GRBVarMap);
                //  System.out.println("l r "+exprs[0] +" "+exprs[1] );
                if (be.isGE()) {
        ...
        try {
            if (e.isIntNum()) {//constant
                ...
                if (e.isAdd()) {
                    ...
                    Expr[] exprs = e.simplify().getArgs(); //simplification (* 1 2 A (*2 A)
                    Double coeff;
                    boolean const0 = isIntConstant(exprs[0]);
                    if (const0) {
                        ...
                        boolean const1 = isIntConstant(exprs[1]);
                        if (const1) {
                            if (exprs[1].isUMinus()) {
...
//e=X*c or c*X
    public static boolean isIntConstant(Expr e) {
        try {
            if (e.isUMinus()) {
