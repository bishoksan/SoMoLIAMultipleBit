...
public class QEBendersDecompWFixedSizeLazy {
    ...
    static final int PRINT_FLAG = 1; //0=do not print, any other number print
...
//        Util.print(PRINT_FLAG, "calling gurobi");
        //System.out.println("calling gurobi");
        //check the constraints in the gurobi solver
        //System.out.println("printing grb constraints name ");
        //grbInterface.printGRBConstraintsName(grbModel);
        int status = grbInterface.optimize(grbModel);
        //System.out.println("gurobi returned");
//        Util.print(PRINT_FLAG, "gurobi returned");

        if (status == GRB.Status.INFEASIBLE) {
//            Util.print(PRINT_FLAG, "querying unsat core");
            //System.out.println("querying unsat core");
            String[] unsatCore = grbInterface.getUnsatCore(grbModel); // = currentAssumptions;
            //System.out.println("unsat core returned ====="+unsatCore.length);
...
//            System.out.println("=================== ma_sat =========== ");
    ...
    public long maxExpr(GurobiInterface grbInterface, GRBEnv env, Context ctx, BitVecExpr e, PreProcessBV ppbv) {
        ...
        GRBLinExpr expr = Util.z3Expr2GRBExpr((ArithExpr) ppbv.convertToLIAExpr(e, ctx).simplify(), grbMaxOpt, z3GRBVarMapLocal);
    ...
    public long minExpr(GurobiInterface grbInterface, GRBEnv env, Context ctx, BitVecExpr e, PreProcessBV ppbv) {
        ...
        GRBLinExpr expr = Util.z3Expr2GRBExpr((ArithExpr) ppbv.convertToLIAExpr(e, ctx).simplify(), grbMinOpt, z3GRBVarMapLocal);
...
//            System.out.println("transformed "+coreId);
            ...
            if (tf.isPrecise() || !tf.isDeltaIntroduced()) {
                ...
                index = getConstraintIndex(coreId);
                ArithFormula af = tf.getFormula().get(index);
                //System.out.println("core index "+index+ " core id "+coreId+" "+af.getFormula());
                if (af.isEquality()) {
...
//                    cutSingleConstr = context.mkNot(context.mkEq(af.getDeltaExpr(), (ArithExpr) Z3Interface.evalExprInModel(previousDeltaModel, af.getDeltaExpr())));
...
//        }
        ...
        while (index < currentAssumptions.length) {
            ...
            if (!isTrueFormula) {
                BoolExpr z3Formula = addConstraintToGrbSolver(id, grbModel, env, grbInterface, context, z3Interface, disjModel, refinementCount);
                //System.out.println("formula added to gurobi (first) "+z3Formula);
                if (!z3Formula.isTrue()) {
                    formulaUnderCurrentAss = z3Formula;
                    index++; //it will be incremented when goes out so that we do not need to explore the same
                    break;
                }
            }
            index++;
        ...
        while (index < currentAssumptions.length) {
            ...
            if (!isTrueFormula) {
                BoolExpr z3Formula = addConstraintToGrbSolver(id, grbModel, env, grbInterface, context, z3Interface, disjModel, refinementCount);
                //System.out.println("formula added to gurobi "+z3Formula);
                if (!z3Formula.isTrue()) {
                    formulaUnderCurrentAss = context.mkAnd(formulaUnderCurrentAss, z3Formula);
                }
            }
...
//        Util.print(PRINT_FLAG, "finished adding constraints");
        //System.out.println("solving arith formula ");
        Witness LIAWitness = solveArithFormula(grbModel, context, z3Interface, grbInterface);
        //System.out.println("arith formula solved");
...
//disjoin qf formulas
            if (qfFormulaCurrentDecomposition.isTrue()) {
                //since we gonna combine qf formuulas disjunctively, we cannot get more than true
                System.out.println("ever here============================");
                return qfFormulaCurrentDecomposition;
            }
...
//        System.out.println("formula under current " + formulaUnderCurrentAss);
        System.out.println("calling elim quantifiers");
        ...
        //System.out.println("formula under current ass "+formulaUnderCurrentAss);
        formulaUnderCurrentAss = z3Interface.simplifyQFLIAFormula(context, formulaUnderCurrentAss);
        //System.out.println("after simplification "+formulaUnderCurrentAss);
...
//            Util.print(PRINT_FLAG, "finished identifying");
            ...
            //System.out.println("disj formula "+disjSolver);
            Model deltaModel = null;
            BoolExpr qfFormula = null;
            varSorts = convertBVSort2IntSort(varSorts, context);
            qfFormula = elimQuantTransformedFormula(varSorts, varNames, falseFormula, z3Interface, context, env, grbInterface, disjSolver, grbModel, deltaModel);
//            System.out.println("QE Res: " + qfFormula);
    ...
    public BoolExpr qeMAFromFile(String inputFile) {
        ...
        System.out.println("the expr "+bExpr);
...
//        }
    ...
    public TransformedFormula getTransformedFormula(GRBModel grbModel, Context context, String id, Z3Interface z3Interface, GurobiInterface grbInterface, GRBEnv env, Solver deltaSolver) {
        ...
        // System.out.println("simplified formula "+simplifiedAaExpr);
        Expr[] exprs = maExpr.getArgs();
...
//        } //else needs quantifier elimination so do not do it on demand
    ...
    public TransformedFormula getTransFormulaLeftDelta(GRBModel grbModel, String id, String inEqSign, boolean isEq, int sortSize, TransformedFormula tf,
            ...
            ArrayList<ArithFormula> arithFormulas, PreProcessBV ppbv) {
        BoolExpr deltaBounds;
        //System.out.println("calling from here "+leftDelta +" "+rightDelta +"ineq "+in);
        ArithExpr deltaExpr = getDeltaExprCut(leftDelta, rightDelta, inEqSign, context);
        arithFormulas.add(new ArithFormula(simplifiedAaExpr, deltaExpr, isEq));
        // System.out.println("actual constr  "+simplifiedAaExpr+" delta "+deltaExpr);
        BoolExpr beLo1 = genLoBound(context, (ArithExpr) deltaInducingExpr, sortSize);
        arithFormulas.add(new ArithFormula(beLo1, leftDelta, false));
        //System.out.println("beLo1 "+beLo1+" delta "+leftDelta);
        BoolExpr beUp1 = genUpBound(context, (ArithExpr) deltaInducingExpr, sortSize);
        arithFormulas.add(new ArithFormula(beUp1, context.mkUnaryMinus(leftDelta), false));
        // System.out.println("beUp1 "+beUp1+" delta "+context.mkUnaryMinus(leftDelta));
        moduloExpr = (BoolExpr) context.mkAnd(maExpr, beLo1, beUp1, moduloExpr).simplify();
        //TODO: if moduloExpr is false, then the formula is not consistent so it can be be replaced by false
        BoolExpr preProcessedExpr = z3Interface.inferDeltaBoundsfromAExpr(context, moduloExpr);
//             System.out.println("preprocessed expr ================= "+ preProcessedExpr);
...
//                System.out.println("calling collect constraints");
    ...
    public void checkEQ(BoolExpr qfFormula, String file) {
        Z3Interface z3Interface = new Z3Interface();
        Context context = z3Interface.getContext();
        BoolExpr bExpr = z3Interface.readSMTInput(file, context);
        Solver s = context.mkSolver();
        s.add(context.mkIff(qfFormula, bExpr));
        Status st = s.check();
        System.out.println("These formulas equivalent " + st);
    }
    ...
    public BoolExpr eliminateMAQuantifiers(String file) {
        ...
        try {
            ...
            System.out.println("QF formula: " + qfFormula.simplify());
            //checkEQ(qfFormula, file);
